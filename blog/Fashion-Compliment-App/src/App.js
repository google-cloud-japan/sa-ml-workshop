...
export const App = (props) => {
  ...
  const resizeImage = async (imageData, width) => {
    ...
    const { naturalHeight: heightCurrent, naturalWidth: widthCurrent } = image;
  ...
  const onFileInputChange = async (evt) => {
    ...
    const image = await resizeImage(evt.target.files[0], 500);

    let chatDataNew = chatData.concat();
    chatDataNew.push({"user": "image", "image": image});
    chatDataNew.push({"user": "bot", "text": "_typing_"});
    setChatData(chatDataNew);

    const data = await getAnswer(image);
...
// eslint-disable-next-line
    ...
    let imageBase64 = await new FileReaderEx().readAsDataURL(image);
    imageBase64 = imageBase64.replace("data:", "").replace(/^.+,/, "");
  ...
  for (const item of chatData) {
    ...
    if (item.user === "image") {
      const imageURL = window.URL.createObjectURL(item.image);
      const elem = (
        <div key={i} className="image" align="right">
          <img src={imageURL} width="200" alt="user provided"/>
        </div>
      );
      chatBody.push(elem);
    }
  ...
  if (buttonDisabled === false) {
    const elem = (
      <div key="fileUpload" className="fileUpload" align="right">
        <button
          onClick={() => inputRef.current.click()}>ファイルアップロード</button>
        <input
          hidden ref={inputRef} type="file" accept="image/*"
